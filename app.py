...
with st.sidebar:
    ...
    if st.session_state.current_page == 'chat':
        ...
        if not history_df.empty and len(history_df.columns) >= 2:
            ...
            # Verifica se tem coluna timestamp, senão usa índice
            for idx, row in recent_conversations.iterrows():
                with st.container():
                    # Usa timestamp se existir, senão usa uma numeração
                    time_info = row.get('timestamp', f'Conversa #{idx + 1}')
                    question = row.get('question', row.iloc[0] if len(row) > 0 else 'N/A')
                    answer = row.get('answer', row.iloc[1] if len(row) > 1 else 'N/A')

                    st.markdown(f"""
                    <div class="chat-history">
                        <small><strong>🕒 {time_info}</strong></small><br>
                        <strong>❓ Pergunta:</strong> {str(question)[:50]}{'...' if len(str(question)) > 50 else ''}<br>
                        <strong>💬 Resposta:</strong> {str(answer)[:50]}{'...' if len(str(answer)) > 50 else ''}
                    </div>
                    """, unsafe_allow_html=True)
